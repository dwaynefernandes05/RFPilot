...
# ============================
def generate_explainable_summary(summary: dict) -> str:
    prompt = f"""
You are generating a short explainable summary for a business user.

Rules:
- 4 to 5 lines maximum
- Clear, professional, non-technical language
...
OUTPUT:
"""
    response = local_llm_call(prompt, max_tokens=120)
...
def master_agent_node(state: RFPState) -> RFPState:
    ...
    # ----------------------------
    # Explainable AI Summary (UI)
    # ----------------------------
    explainable_summary = generate_explainable_summary(
        state["master_output"]["summaries"]["technical"]
    )

    state["master_output"]["summaries"]["technical_explainable"] = explainable_summary
    ...
    print("\n========== EXPLAINABLE TECHNICAL SUMMARY ==========")
    print(state["master_output"]["summaries"]["technical_explainable"])
    print("======================================\n")
...
# ============================
#some things for the tech agent
# ============================
'''
def extract_specs_llm_fast(spec_text: str) -> dict:
    """
    Uses LLM to convert human-readable specs into key-value pairs.
    """
    prompt = f"""
...
Example keys:
voltage_kv, conductor_material, insulation_type, core_count, size_sqmm, standard
...
def canonicalize_rfp_specs(raw_specs: dict) -> dict:
    """
    Normalizes spec keys & values so matching is consistent.
    """
    canonical = {}
    ...
    for k, v in raw_specs.items():
        ...
        if "volt" in key:
            canonical["voltage_kv"] = str(v).replace("kv", "").strip()
        elif "conductor" in key:
            canonical["conductor_material"] = v.lower()
        elif "insulation" in key:
            canonical["insulation_type"] = v.lower()
        elif "core" in key:
            canonical["core_count"] = str(v)
        elif "sqmm" in key or "size" in key:
            canonical["size_sqmm"] = str(v)
        elif "standard" in key:
            canonical["standard"] = str(v)

    return canonical

def compute_weighted_match(rfp_specs: dict, sku_specs: dict) -> float | None:
    """
    Computes equal-weight spec match percentage.
    """
    if not rfp_specs or not sku_specs:
        return None
    ...
    for key, rfp_value in rfp_specs.items():
        sku_value = sku_specs.get(key)

        if sku_value is None:
            continue

        if str(rfp_value).lower() == str(sku_value).lower():
            matched += 1
